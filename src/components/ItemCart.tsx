  ...
  return (
    <div className="flex items-center space-x-4 p-2 text-xs relative">
      ...
      <div className="flex-grow flex flex-col gap-y-1">
        ...
        <p className=" text-gray-500">Precio total: <span className="font-semibold">${discountedPrice.toFixed(2)}{productInCart.productQuantity > 3 && (<span className="text-green-500 text-xs ml-1">(10% Off)</span>)}</span></p>
      </div>
    </div>
  )
}
